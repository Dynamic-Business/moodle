...
class theme_dynamicbase_core_renderer extends core_renderer {
...
<?php }
    ...
    protected function is_administrator(){
        ...
        $data = $DB->get_field_sql($sql,array('siteadmins'));
        $arr = explode(',',$data);
        $exists = array_search($USER->id,$arr);
        return ($exists === FALSE) ? false : true;
    }

    protected function is_groupmanager(){
        global $CFG,$DB,$USER;
        $sql = "SELECT userid FROM {role_assignments} WHERE roleid = ? AND userid = ?";
        $data = $DB->get_record_sql($sql,array(99,$USER->id));
        if($data === false){
            return false;
        }
        return true;
    ...
    public function navbar() {
        ...
        foreach ($breadcrumbs as $key) {
            $list_items .= "<li>{$key}";
            if(++$i !== $numItems) {
                $list_items .= "<div class='divider'>{$divider}</div>";
            }
            $list_items .= "</li>";
        }
        //$list_items = "<li>".join($breadcrumbs," $divider</li>"); //'<li>'.join(" $divider</li><li>", $breadcrumbs).'</li>';
        $title = '<span class="accesshide">'.get_string('pagepath').'</span>';
        return $title . "<ul class=\"nav bread-crumb\">$list_items</ul>";
    }
    ...
    public function body_attributes($additionalclasses = array()) {
        ...
        return ' id="'. $this->body_id().'" class="'.$this->body_css_classes($additionalclasses).' '.$this->develeopment_css_class().'"';
    }

    protected function develeopment_css_class(){
        return ($this->debugging === true) ? "dev" : "";
    }
}
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_dynamicbase_core_renderer_maintenance extends core_renderer_maintenance {
    /**
